...
namespace Avalonia.Labs.Notifications.Windows
{
    ...
    internal class NativeNotification : INativeNotification
    {
        ...
        public bool AddProgressBar { get; set; }
        ...
        public void Show()
        {
            ...
            var dateTimeTs = DateTime.Now.AddSeconds(15).ToFileTime();
            unsafe
            {
                WDateTime w = new WDateTime();
                w.UniversalTime = dateTimeTs;
                var x = CurrentNotification?.ExpirationTime;
               var x2 = CurrentNotification.Content;

//                CurrentNotification.SetExpirationTime(w);
              //  CurrentNotification?.SetExpirationTime(&w);
            }
