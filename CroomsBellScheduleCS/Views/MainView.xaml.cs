...
public sealed partial class MainView : UserControl
{
    ...
    private async Task Init()
    {
        try
        {
            ...
            MainWindow.Instance.SetTitleBar((UIElement?)Content);
 await SettingsManager.LoadSettings();
    ...
    public async Task SetTaskbarMode(bool showInTaskbar)
    {
        ...
        if (MainWindow.Instance.AppWindow != null)
            _windowApp = MainWindow.Instance.AppWindow;

        if (OperatingSystem.IsWindows())
        {
            handle = WindowNative.GetWindowHandle(MainWindow.Instance);
    ...
    public void PositionWindow()
    {
        //nint handle = WindowNative.GetWindowHandle(MainWindow.Instance);
        // WindowId id = Win32Interop.GetWindowIdFromWindow(handle);
       /* AppWindow appWindow = MainWindow.Instance.AppWindow;//AppWindow.GetFromWindowId(id);
        if (appWindow != null)
        ...
        _windowApp.MoveAndResize(new RectInt32()
        {
            X = mWidth - _windowApp.Size.Width - 20,
            Y = mHeight - _windowApp.Size.Height - 20,
            Width = GetDpi() * 4,
            Height = GetDpi()
        });*/
    }
